...
class MiniGPTBase(BaseModel):
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts, lengths=None): # 用于整合prompt+image的嵌入
        # 相当于分三种情况，分别是有没有prompt和图片
        if prompts is None or len(prompts) == 0:
            # prompts is not provided, just return the original image embedding
            return img_embeds, atts_img
        elif img_embeds is None:
            # prompt is provided but there is no image embedding. return the prompt embedding in right padding
            self.llama_tokenizer.padding_side = "right" # 右侧padding，即不满足长度在右侧补全
            ...
            if isinstance(prompts, str):
                ...
                # 假设 prompts = ['Prompt 1', 'Prompt 2'] 和 img_embeds = [img1, img2, img3, img4]
                # [
                # ['Prompt 1', 'Prompt 2'],
                # ['Prompt 1', 'Prompt 2'],
                # ['Prompt 1', 'Prompt 2'],
            ...
            # zip生成元组[(embed1, 'Prompt 1'), (embed2, 'Prompt 2')]
    ...
    def preparing_embedding(self, samples):
        ...
        if 'image' in samples:
            img_embeds, img_atts = self.encode_img(samples["image"]) # 定义在minigpt4.py
        else:
        ...
        if 'conv_q' in samples:
            ...
            conv_q = [[self.prompt_template.format(item) for item in items] for items in conv_q] # 把多个对话按模板处理好放进去
    ...
    def forward(self, samples, reduction='mean'):
        # prepare the embedding to condition and the embedding to regress 条件化和回归训练
    ...
    def generate(
        ...
        image_lists = [[image_emb[None]] for image_emb in img_embeds] # image_embp[None]和unsqueeze的效果相同，这里把一维变成2维
